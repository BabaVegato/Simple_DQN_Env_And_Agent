...
def Display_World(world, win, obs, init):
	...
	if init :
		win = GraphWin("Environment", 1200, 1600, autoflush=False)
	...
	for i in range(world.SIZE_OBS*2 + 1):
		for j in range(world.SIZE_OBS*2 + 1):
			rect = Rectangle(Point(RECT_SIZE*i + offsetx, RECT_SIZE*j + offsety), 
				Point(RECT_SIZE*(i+1) + offsetx, RECT_SIZE*(j+1) + offsety))
